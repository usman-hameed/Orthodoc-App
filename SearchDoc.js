      ...
      <View style={styles.searchResults}>
        <FlatList
          ...
          renderItem={(item) => {
            ...
            if (item.item.email?.includes(search) ||item.item.displayName?.includes(search)  ||item.item.address[0][0].city?.includes(search) ) {
              return (
                ...
                <View style={{paddingHorizontal:10,paddingVertical:20, marginHorizontal:10, backgroundColor:"#f1f5f9", borderRadius:14, marginBottom:3  }}>
                  ...
                  <Text>
                  {`${item.item.address[0][0].city}, ${item.item.address[0][0].district}`}
                  </Text>
                </View>
                </TouchableOpacity>
              );
